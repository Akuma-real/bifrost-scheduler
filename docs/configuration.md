...
- 静态配置优先表达价格，也就是 `price_rmb_per_dao`。同一个 pool 的可用 provider 价格都填全后，`cost_weight` 会自动按最低价换算。
  ...
  "pools": [
    {
      ...
      "providers": [
        {
          "name": "provider_a",
          "price_rmb_per_dao": 0.045
        },
        {
          "name": "provider_b",
          "price_rmb_per_dao": 0.09
        },
        {
          "name": "new_provider_c",
          "role": "candidate",
          "price_rmb_per_dao": 0.225,
          "min_weight": 0.05
        },
        {
    ...
    },
    {
      ...
      "providers": [
        {
          "name": "provider_e",
          "price_rmb_per_dao": 0.05
        },
        {
          "name": "provider_f",
          "price_rmb_per_dao": 0.1
        }
      ]
    }
  ]
...
- `price_rmb_per_dao`：渠道价格，单位 RMB/刀。同池价格填全时，调度器自动换算 `cost_weight`。
- `probe.enabled: false`：默认不发额外测速请求。开启后才会用首字速度参与明确异常判断；真实业务样本也足够时，才会按首字速度重排健康权重。
...
| `samples` | integer | 否 | `3` | `1` 到 `5` | 每个 provider 每轮测速次数。上限 5 是为了防止误配置导致费用过高。 |
...
- 本轮没有 `warning` / `critical` 问题决策：不发送通知。健康恢复和普通调权这类 `info` 决策只写日志和最近计划，不主动推送。
- `plan` 命令：只要本轮有 `decisions`，发送一次。
- `daemon` 命令：只有变更指纹变化时才发送；同一批变更不会每个 interval 重复刷屏。
- 动作较多或消息较长时：自动拆成多条 Telegram 发送，不会只显示前几条。
- Telegram 发送失败：只写日志，不阻断 Bifrost 调度。
- `BIFROST_SCHEDULER_TG_INTERACTIVE=true` 时，daemon 会额外启动 Telegram 长轮询，读取常驻键盘按钮文本。
- Telegram 交互只允许 `BIFROST_SCHEDULER_TG_CHAT_ID` 对应的 chat 操作；其他 chat 的消息会被拒绝。
- Telegram 交互不提供直接写线上按钮。“立即 dry-run”永远是 dry-run。
- 调整价格的主入口是常驻键盘：点“价格” -> “调整价格” -> 选 key -> 选渠道 -> 输入 `0.055 RMB/刀` -> 确认。
- Telegram 交互回复使用 HTML 富文本；执行“立即 dry-run”这类慢操作时会发送 `typing` 状态，让客户端显示“正在输入...”。
...
| 按钮 | 作用 | 是否写 Bifrost |
| --- | --- | --- |
| “状态” | 查看 daemon 是否运行、运行间隔、最近错误、最近决策数。 | 否 |
| “最近计划” | 查看最近一次调度计划摘要。 | 否 |
| “立即 dry-run” | 立即执行一次手动 dry-run 预览。 | 否 |
| “价格” | 查看当前配置里的 RMB/刀 价格和派生 `cost_weight`，并进入按钮式调价。 | 否 |
| “调整价格” | 选择 key 和 provider，然后输入 RMB/刀 价格，确认后写调度器配置。 | 不直接写 Bifrost |
| “静音 1h” | 静音 Telegram 变更通知 1 小时。调度器仍继续运行。 | 否 |
| “恢复通知” | 恢复 Telegram 变更通知。 | 否 |

为什么“立即 dry-run”不写线上：

- Telegram 交互适合临时查看和手动预览。价格确认后只改调度器配置文件，不直接调用 Bifrost。
- 自动写入仍然只由 daemon 自己的运行参数控制：`config.json` 必须是 `guarded_write`，并且 daemon 命令必须带 `--apply`。
- 这样即使 Telegram bot token 泄露，攻击面也不会直接变成 Bifrost 写入入口。
...
| `price_rmb_per_dao` | number | 否 | 无 | 正数 | provider 价格，单位 RMB/刀。同一个 pool 的可用 provider 都填写时，自动换算 `cost_weight = 同池最低价格 / 当前价格`。 |
| `cost_weight` | number | 否 | `rules.default_cost_weight`，默认 `1` | `0` 或正数 | provider 健康时恢复到的目标权重。通常不用手写；只有未填全 `price_rmb_per_dao` 时才作为手动兜底。 |
...
### 价格和 `cost_weight` 到底是什么

`price_rmb_per_dao` 是源数据，单位 RMB/刀。`cost_weight` 不是质量分，它只是健康状态下的目标权重。
...
如果同一个 pool 的可用 provider 都写了 `price_rmb_per_dao`，调度器会自动换算：

```text
cost_weight = 同池最低 price_rmb_per_dao / 当前 provider 的 price_rmb_per_dao
```

如果价格没填全，调度器不会自动覆盖手写的 `cost_weight`。

质量由运行日志自动调度：

- 健康：恢复到 `cost_weight`。
- 开启主动测速、主动样本和真实业务样本都足够：先按首字速度重新计算目标权重，再考虑恢复到 `cost_weight`。
...
| `min_probe_samples` | integer | 否 | `3` | 正整数 | 至少有这么多主动测速成功样本时，首字结果才参与调权。 |
...
| 动作 | 会做什么 | 触发原因 |
| --- | --- | --- |
| `set_weight` | 修改某个 VK provider config 的权重 | 健康恢复、错误率超阈值但未满足连续坏窗口、P95 延迟连续慢窗口达标、主动首字测速和真实业务样本都足够后的重新分配、样本太少但需要探测。 |
