...
| `/prices` | 查看当前配置里的 RMB/刀 价格和派生 `cost_weight`。 | 否 |
| `/price pool provider 0.055` | 预览修改某个渠道的 RMB/刀 价格，确认后写调度器配置。`pool` 可以是调度器 pool id，也可以是 Bifrost `virtual_key`。 | 不直接写 Bifrost |
