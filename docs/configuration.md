...
- 调整价格的主入口是按钮：点“价格” -> “调整价格” -> 选 key -> 选渠道 -> 输入 `0.055 RMB/刀` -> 确认。
...
| `/prices` 或“价格”按钮 | 查看当前配置里的 RMB/刀 价格和派生 `cost_weight`，并进入按钮式调价。 | 否 |
| “调整价格”按钮 | 选择 key 和 provider，然后输入 RMB/刀 价格，确认后写调度器配置。 | 不直接写 Bifrost |
| `/price pool provider 0.055` | 命令兜底入口；`pool` 可以是调度器 pool id，也可以是 Bifrost `virtual_key`。 | 不直接写 Bifrost |
...
- Telegram 交互适合临时查看和手动预览。`/price` 确认后只改调度器配置文件，不直接调用 Bifrost。
