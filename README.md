...
调度器不按 `low` 或 `stable` 写死不同策略。配置里只需要给 provider 写健康时的 `cost_weight`；质量由 Bifrost 近期日志自动判断。需要首字优先时，可以开启主动流式测速，调度器会用 `provider/model` 定向测每个 provider 的真实 TTFT。
...
- 可选 Telegram bot 通知：本轮有变更时发送变更摘要；动作较多时会自动分页，不会省略后续动作；daemon 模式会按变更指纹去重，避免同一批变更每 5 分钟重复刷屏。
...
通知失败不会阻断调度，只会写一条错误日志。没有建议动作时不发通知。动作较多或消息较长时，Telegram 通知会自动拆成多条发送，交互按钮只放在最后一条。
