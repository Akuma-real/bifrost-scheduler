...
调度器不按 `low` 或 `stable` 写死不同策略。配置里优先给 provider 写 `price_rmb_per_dao`，调度器会自动换算健康时的 `cost_weight`；质量由 Bifrost 近期日志自动判断。需要首字优先时，可以开启主动流式测速，调度器会用 `provider/model` 定向测每个 provider 的真实 TTFT。
...
- 可选主动首字测速：用 `provider/model` 定向发流式测试请求，记录真实首字 P95 `p95_ttft_ms`，按“首字优先，其次成本”调整健康目标权重。
...
- 用样本量、错误数和连续坏窗口做保护，避免单次失败或低样本误判；Bifrost 日志 P95 延迟默认只展示，不自动调权。
- 同一轮计划会按“计划后的活跃 provider 数”保护 `min_active_providers`，避免多个坏渠道同时被清零导致池子无可用渠道。
- 可选 Telegram bot 通知：本轮有 `warning` / `critical` 问题决策时发送摘要；健康恢复和普通调权只写日志，不主动推送；动作较多时会自动分页，不会省略后续动作；daemon 模式会按变更指纹去重，避免同一批问题每 5 分钟重复刷屏。
...
主动首字测速默认关闭。开启后会产生少量真实模型请求，建议先只读观察：
...
报告里的 `首字 P95 ms` 来自主动流式探测，不是 Bifrost `/api/logs` 的总耗时 `latency`。
...
Telegram 通知是可选的。只要配置 bot token 和 chat id，调度器发现本轮有 `warning` / `critical` 问题决策时会发通知：
